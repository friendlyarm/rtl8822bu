...
ccflags-y += -Wno-vla -Wno-uninitialized -Wno-bool-operation
ccflags-y += -Wno-misleading-indentation -Wno-implicit-fallthrough
...
ifeq ($(CONFIG_PLATFORM_SNI), y)
ccflags-y += -DCONFIG_PLATFORM_SNI
ccflags-y += -DCONFIG_LITTLE_ENDIAN
# default setting for Android 4.1, 4.2
ccflags-y += -DCONFIG_CONCURRENT_MODE
ccflags-y += -DCONFIG_IOCTL_CFG80211 -DRTW_USE_CFG80211_STA_EVENT
...
KSRC := $(KERNEL_DIR)
# Workarround the duplicate module name
MODULE_NAME := rtl8822bu
endif
...
MFLAGS := ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) -C $(KSRC) M=$(CURDIR) V=$(V)
