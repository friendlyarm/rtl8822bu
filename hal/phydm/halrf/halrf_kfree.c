...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,"[kfree] 8821c 0x55[19:14]=0x%X 0x65[19:14]=0x%X\n",
		...
		odm_get_rf_reg(dm, e_rf_path, RF_0x65, (BIT(19) | BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)))
		);
}
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_MP,"[kfree] 8822b 2g tx pa bias rf_0x3f(0x%X) path=%d\n",
		odm_get_rf_reg(dm, e_rf_path, RF_0x3f, (BIT(12) | BIT(11) | BIT(10) | BIT(9))), e_rf_path);
}
...
void
phydm_get_power_trim_offset_8192f(
	void	*dm_void
)
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;
	...
	ODM_RT_TRACE(dm, ODM_COMP_MP, ODM_DBG_LOUD, ("[kfree] 8192F power trim flag:0x%02x\n", power_trim_info->flag));

	if (power_trim_info->flag & KFREE_FLAG_ON) {
		for (i = 0; i < KFREE_CH_NUM; i++) {
			for (j = 0; j < 2; j++)
				ODM_RT_TRACE(dm, ODM_COMP_MP, ODM_DBG_LOUD, ("[kfree] 8192F power_trim_data->bb_gain[%d][%d]=0x%X\n", i, j, power_trim_info->bb_gain[i][j]));
		}
	}

}
void
phydm_set_kfree_to_rf_8192f(
	void		*dm_void,
	u8		e_rf_path,
	u8		channel_idx,
	u8		data
)
{
	...
	if(e_rf_path == 0){
		...
		odm_set_rf_reg(dm, e_rf_path, RF_0x33, BIT(0), (data & BIT(0)));
		odm_set_rf_reg(dm, e_rf_path, RF_0x33, ( BIT(3) | BIT(2) | BIT(1)), ((data & 0xf) >> 1));
		...
		if (channel_idx == 0){
		odm_set_rf_reg(dm, e_rf_path, RF_0x33, 0x70000, 0);
		odm_set_rf_reg(dm, e_rf_path, RF_0x33, BIT(0), (data & BIT(4)));
		odm_set_rf_reg(dm, e_rf_path, RF_0x33, ( BIT(3) | BIT(2) | BIT(1)), ((data & 0xf0) >> 5));
...
)
{
	...
	if (cali_info->reg_rf_kfree_enable == 2) {
		...
		if (dm->support_ic_type &ODM_RTL8710B)
			...
			for (i = 0; i < KFREE_CH_NUM; i++){
				for (j = 0; j < max_rf_path; j++)
					ODM_RT_TRACE(dm, ODM_COMP_MP, ODM_DBG_LOUD, ("[kfree] power_trim_data->bb_gain[%d][%d]=0x%X\n", i, j, power_trim_info->bb_gain[i][j]));
			}
		}
		if (*dm->band_type == ODM_BAND_2_4G && power_trim_info->flag & KFREE_FLAG_ON_2G){
			if (!(dm->support_ic_type & ODM_RTL8192F)){
				...
				if (channel_to_sw >= 10 && channel_to_sw <= 14)
					channel_idx = 2;
				for (rfpath = RF_PATH_A;  rfpath < max_rf_path; rfpath++){
					ODM_RT_TRACE(dm, ODM_COMP_MP, ODM_DBG_LOUD, ("[8192F kfree] phydm_kfree(): channel_to_sw=%d PATH_%d bb_gain:0x%X\n", channel_to_sw, rfpath, power_trim_info->bb_gain[channel_idx][rfpath]));
