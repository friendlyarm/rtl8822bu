...
)
{
	...
	if (dm->rf_calibrate_info.iqk_step == 7) {
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION,"[IQK]PathA_GS1_retry = %d, PathA_GS2_retry = %d, PathB_GS1_retry = %d, PathB_GS2_retry = %d\n",
			iqk_info->gs_retry_count[0][RF_PATH_A][RXIQK1], iqk_info->gs_retry_count[0][RF_PATH_A][RXIQK2],
			iqk_info->gs_retry_count[0][RF_PATH_B][RXIQK1], iqk_info->gs_retry_count[0][RF_PATH_B][RXIQK2]);
		for (i = 0; i < 2; i++) {
