...
)
{
	...
	if (dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D | ODM_RTL8710B)) {
		dm->cck_agc_report_type = odm_get_bb_reg(dm, R_0x950, BIT(11)) ? 1 : 0; /*1: 4bit LNA, 0: 3bit LNA */
	...
	if (dm->support_ic_type & ODM_RTL8821C) {
		dm->cck_new_agc = odm_get_bb_reg(dm, R_0xa9c, BIT(17)) ? true : false;          /*1: new agc  0: old agc*/
		if (dm->cck_new_agc == 0 && dm->default_rf_set_8821c == SWITCH_TO_BTG)
			dm->cck_agc_report_type = 1;
	}
#endif

#if ((RTL8723D_SUPPORT == 1) || (RTL8822B_SUPPORT == 1) || (RTL8197F_SUPPORT == 1) || (RTL8710B_SUPPORT == 1) || (RTL8192F_SUPPORT == 1))/*jj add 20170822*/
	if (dm->support_ic_type & (ODM_RTL8723D | ODM_RTL8822B | ODM_RTL8197F | ODM_RTL8710B | ODM_RTL8192F))/*jj add 20170822*/
		dm->cck_new_agc = odm_get_bb_reg(dm, R_0xa9c, BIT(17)) ? true : false;          /*1: new agc  0: old agc*/
	else
#endif
	{
...
)
{
	...
	if (dm_value[0] == 100) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "05. (( %s ))CCK_PD\n",
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "14. (( %s ))CFO_TRACK\n",
			       ((dm->support_ability & ODM_BB_CFO_TRACKING) ? ("V") : (".")));
		PDM_SNPF(out_len, used, output + used, out_len - used, "15. (( %s ))ENV_MONITOR\n",
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "18. (( %s ))NA_SAT_CHK\n",
			       ((dm->support_ability & ODM_BB_LNA_SAT_CHK) ? ("V") : (".")));
		PDM_SNPF(out_len, used, output + used, out_len - used, "19. (( %s ))DYN_RX_PATH\n",
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "01. (( %s ))RF IQK OFFLOAD\n",
			       ((dm->fw_offload_ability & PHYDM_RF_IQK_OFFLOAD) ? ("V") : (".")));
		PDM_SNPF(out_len, used, output + used, out_len - used, "================================\n");

	} else if (dm_value[0] == 101) {
		dm->support_ability = 0;
		PDM_SNPF(out_len, used, output + used, out_len - used, "Disable all support_ability components\n");
	} else {
		if (dm_value[1] == 1) { /* enable */
			...
			PDM_SNPF(out_len, used, output + used, out_len - used, "[Warning!!!]  1:enable,  2:disable\n");
	}
	PDM_SNPF(out_len, used, output + used, out_len - used, "pre-supportability = 0x%llx\n",
		         pre_support_ability);
	PDM_SNPF(out_len, used, output + used, out_len - used, "Cur-supportability = 0x%llx\n",
		       dm->support_ability);
	PDM_SNPF(out_len, used, output + used, out_len - used, "================================\n");
	...
	if ((pause_type == PHYDM_PAUSE) || (pause_type == PHYDM_PAUSE_NO_SET)) {
		...
		*pause_lv_pre = pause_lv;
		...
		if ((dm->pause_ability & pause_func_bitmap) == 0) {
			PHYDM_DBG(dm, ODM_COMP_API, "[RESUME] No Need to Revert\n");
			return PAUSE_SUCCESS;
		}
...
)
{
	...
	for (path = RF_PATH_A; path < PHYDM_MAX_RF_PATH; path++) {
		...
		if (dm->support_ic_type & ODM_IC_11N_SERIES)
			odm_set_bb_reg(dm, R_0x88c, BIT(21)|BIT(20), 0x3);
		else {
			odm_set_bb_reg(dm, R_0xc00, BIT(1)|BIT(0), 0x0);
			if (dm->rf_type > RF_1T1R)
				odm_set_bb_reg(dm, R_0xe00, BIT(1)|BIT(0), 0x0);
		}
		...
		if (dm->support_ic_type & ODM_IC_11N_SERIES) {
			odm_set_bb_reg(dm, R_0x88c, BIT(21)|BIT(20), 0x0);
		} else {
			odm_set_bb_reg(dm, R_0xc00, BIT(1)|BIT(0), 0x3);
			odm_set_bb_reg(dm, R_0xe00, BIT(1)|BIT(0), 0x3);
		}
	...
	if (dm->support_ic_type & (ODM_RTL8188F | ODM_RTL8710B)) {
		...
		odm_set_bb_reg(dm, R_0xc10, 0x3c000000, ((0x3c0 & offset_i_hex[0]) >> 6));
		odm_set_bb_reg(dm, R_0xc10, 0xfc00, (0x3f & offset_i_hex[0]));
		odm_set_bb_reg(dm, R_0xc14, 0x3c000000, ((0x3c0 & offset_q_hex[0]) >> 6));
		...
		/*Before filling into registers, offset should be multiplexed (-1)*/
			...
			odm_set_bb_reg(dm, R_0xe10, 0x3c000000, ((0x3c0 & offset_i_hex[1]) >> 6));
			odm_set_bb_reg(dm, R_0xe10, 0xfc00, (0x3f & offset_i_hex[1]));
			odm_set_bb_reg(dm, R_0xe14, 0x3c000000, ((0x3c0 & offset_q_hex[1]) >> 6));
			odm_set_bb_reg(dm, R_0xe14, 0xfc00, (0x3f & offset_q_hex[1]));
		}
	}
#endif
}
...
)
{
	...
	u8	bw = *dm->band_width;
	u32	bb_regf0 = odm_get_bb_reg(dm, R_0xf0, MASKDWORD);
