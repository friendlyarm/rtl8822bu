...
)
{
	...
	adaptivity->adapt_igi_up = IGI - ADAPT_DC_BACKOFF;
	adaptivity->h2l_lb = th_h2l_dmc + ADAPT_DC_BACKOFF;
	adaptivity->l2h_lb = th_l2h_dmc + ADAPT_DC_BACKOFF;
...
)
{
	...
	if (!adaptivity->edcca_en || adaptivity->is_stop_edcca) {
	...
	if ((!(dm->support_ability & ODM_BB_ADAPTIVITY)) && adaptivity->debug_mode == false) {
		...
		dm->th_l2h_ini = adaptivity->th_l2h_ini_mode2;
		dm->th_edcca_hl_diff = adaptivity->th_edcca_hl_diff_mode2;
	}
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	...
	PHYDM_DBG(dm, DBG_ADPTVTY, "adajust_igi_level= 0x%x, is_adapt_en = %d\n",
		adaptivity->adajust_igi_level, adaptivity->is_adapt_en);

	if (dm->support_ic_type & (ODM_IC_11AC_GAIN_IDX_EDCCA | ODM_IC_11N_GAIN_IDX_EDCCA)) {
		if ((adaptivity->adajust_igi_level > igi) && adaptivity->is_adapt_en)
			diff = adaptivity->adajust_igi_level - igi;
		else if (!adaptivity->is_adapt_en)
			diff = 0x3e - igi;
		...
		diff = igi_target - (s8)igi;
		th_l2h_dmc = dm->th_l2h_ini + diff;
		if (th_l2h_dmc > 10 && adaptivity->is_adapt_en)
			th_l2h_dmc = 10;
...
)
{
	...
	if (dm_value[0] == PHYDM_ADAPT_DEBUG) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "th_l2h_ini_debug = %d\n",
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "EDCCA Threshold Pause\n");
		adaptivity->edcca_en = false;
	} else if (dm_value[0] == PHYDM_EDCCA_RESUME) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "EDCCA Resume\n");
		adaptivity->edcca_en = true;
	} else if (dm_value[0] == PHYDM_ADAPT_MSG) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "debug_mode = %s, th_l2h_ini = %d\n",
			       (adaptivity->debug_mode ? "TRUE" : "FALSE"), dm->th_l2h_ini);
		if (dm->support_ic_type & ODM_IC_11N_SERIES) {
...
#endif
		if (h2l_diff == 7)
			PDM_SNPF(out_len, used, output + used, out_len - used, "adaptivity enable\n");
		else
			PDM_SNPF(out_len, used, output + used, out_len - used, "adaptivity disable\n");
	}
	*_used = used;
	*_out_len = out_len;
}
