...
)
{
	...
	for (i = 0; i < (adc_smp_buf->length >> 2) - 2; i += 2) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "%08x%08x\n",
			adc_smp_buf->octet[i], adc_smp_buf->octet[i + 1]);
	}
...
)
{
	...
	PHYDM_DBG(dm, DBG_TMP, "start_addr = ((0x%x)), end_addr = ((0x%x)), buffer_size = ((%d))\n",
		  adc_smp_buf->start_pos, adc_smp_buf->end_pos, adc_smp_buf->buffer_size);
}
...
)
{
	...
	if (dm->support_ic_type & PHYDM_IC_SUPPORT_LA_MODE) {
		...
		if ((strcmp(input[1], help) == 0)) {
			...
			PDM_SNPF(out_len, used, output + used, out_len - used, "a.En= ((1)),  b.mode = ((%d)), c.Trig_Sel = ((0x%x)), d.Dma_type = ((%d))\n",
				       trig_mode, trig_sig_sel, dma_data_sig_sel);
			PDM_SNPF(out_len, used, output + used, out_len - used, "e.Trig_Time = ((%dus)), f.Dbg_head/mac_ref_mask = ((0x%x)), g.dbg_port = ((0x%x))\n",
				       trigger_time_mu_sec, adc_smp->la_mac_mask_or_hdr_sel,
				       adc_smp->la_dbg_port);
			PDM_SNPF(out_len, used, output + used, out_len - used, "h.Trig_edge = ((%d)), i.smp rate = ((%d MHz)), j.Cap_num = ((%d))\n",
			...
			adc_smp_stop(dm);
			PDM_SNPF(out_len, used, output + used, out_len - used, "Disable LA mode\n");
		}
	}
	*_used = used;
	*_out_len = out_len;
