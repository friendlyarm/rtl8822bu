...
void
phydm_trx_hw_ant_div_init_22b(
	void		*dm_void
)
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;

	PHYDM_DBG(dm, DBG_ANT_DIV, "***8822B AntDiv_Init =>  ant_div_type=[CG_TRX_HW_ANTDIV]\n");
	...
	odm_set_bb_reg(dm, R_0x1990, BIT(17) | BIT(16), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_8) */
	odm_set_bb_reg(dm, R_0x1990, BIT(19) | BIT(18), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_9) */
	...
	odm_set_bb_reg(dm, R_0x1990, BIT(11) | BIT(10), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_5) */
	odm_set_bb_reg(dm, R_0x1990, BIT(21) | BIT(20), 0x0);/* r_rfe_path_sel_   (RFE_CTRL_10) */
	odm_set_bb_reg(dm, R_0xcbc, BIT(19), 0x0);	/* RFE_buffer */
	odm_set_bb_reg(dm, R_0xcbc, BIT(5), 0x0);	/* rfe_inv  (RFE_CTRL_8) */
	odm_set_bb_reg(dm, R_0xcbc, BIT(10), 0x1);	/* rfe_inv  (RFE_CTRL_9) */
	odm_set_bb_reg(dm, R_0xcb0, 0xF00000, 0x8);		/* path-A, RFE_CTRL_5 */
	odm_set_bb_reg(dm, R_0xcb4, 0xF00, 0x8);		/* path-A, RFE_CTRL_9 */
	/* Pin Settings */
...
#endif
	PHYDM_DBG(dm, DBG_ANT_DIV, "***8197F AntDiv_Init =>  ant_div_type=[CGCS_RX_HW_ANTDIV]\n");
...
)
{
	...
	/* 2 [--92E---] */
		...
		if (dm->ant_div_type != CGCS_RX_HW_ANTDIV) {
			PHYDM_DBG(dm, DBG_ANT_DIV, "[Return!!!]  8197F Not Supprrt This AntDiv type\n");
			dm->support_ability &= ~(ODM_BB_ANT_DIV);
			return;
		}
...
)
{
	...
	if (dm_value[0] == 1) { /*fixed or auto antenna*/
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "AntDiv_period=((%d))\n",
			dm->antdiv_period);
	}
	*_used = used;
	*_out_len = out_len;
