...
)
{
	...
	if (dm->support_ic_type & (ODM_RTL8821 | ODM_RTL8881A)) {
		...
		odm_set_bb_reg(dm, R_0xcb4, 0xFF, 77); /*DPDT_N = 1b'0*/  /*DPDT_P = 1b'0*/
...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(dm, R_0xc40, BIT(9), reg_value);
		PHYDM_DBG(dm, ODM_COMP_API, "Enable NBI Reg0xC40[9] = ((0x%x))\n", reg_value);

	} else if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (dm->support_ic_type & (ODM_RTL8822B|ODM_RTL8821C)) {
			odm_set_bb_reg(dm, R_0x87c, BIT(13), reg_value);
			odm_set_bb_reg(dm, R_0xc20, BIT(28), reg_value);
			if (dm->rf_type > RF_1T1R)
				odm_set_bb_reg(dm, R_0xe20, BIT(28), reg_value);
		} else
			odm_set_bb_reg(dm, R_0x87c, BIT(13), reg_value);
...
)
{
	...
	if (function_map == PHYDM_API_NBI) {
		if (dm_value[0] == 100) {
			...
			PDM_SNPF(out_len, used, output + used, out_len - used, "[Disable NBI]\n");
			set_result = phydm_nbi_setting(dm, FUNC_DISABLE, channel, bw, f_interference, second_ch);

		} else

			set_result = PHYDM_SET_FAIL;
		PDM_SNPF(out_len, used, output + used, out_len - used, "[NBI set result: %s]\n",
	...
	/*-------------------------------------------------------------------------------------------------------------------------------*/
		...
		if (dm_value[0] == 100) {
			PDM_SNPF(out_len, used, output + used, out_len - used, "[HELP-CSI MASK]  EN(on=1, off=2)   CH   BW(20/40/80)  f_intf(Mhz)    Scnd_CH(L=1, H=2)\n");
			return;

		} else if (dm_value[0] == FUNC_ENABLE) {
			PDM_SNPF(out_len, used, output + used, out_len - used, "[Enable CSI MASK] CH = ((%d)), BW = ((%d)), f_intf = ((%d)), Scnd_CH = ((%s))\n",
				       channel, bw, f_interference,
			...
			PDM_SNPF(out_len, used, output + used, out_len - used, "[Disable CSI MASK]\n");
			set_result = phydm_csi_mask_setting(dm, FUNC_DISABLE, channel, bw, f_interference, second_ch);

		} else

			set_result = PHYDM_SET_FAIL;
		PDM_SNPF(out_len, used, output + used, out_len - used, "[CSI MASK set result: %s]\n",
			       (set_result == PHYDM_SET_SUCCESS) ? "Success" : ((set_result == PHYDM_SET_NO_NEED) ? "No need" : "Error"));
	}
	*_used = used;
	*_out_len = out_len;
...
)
{
	...
	if ((path > RF_PATH_B) || (hw_rate > ODM_RATEMCS15)) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "config_phydm_read_txagc_n(): unsupported path (%d)\n", path);
	...
	if (path == RF_PATH_A) {
		switch (hw_rate) {
		case ODM_RATE1M:
			reg_txagc = REG_TX_AGC_A_CCK_1_MCS32;
			reg_mask = 0x00007f00;
			break;
		case ODM_RATE2M:
			reg_txagc = REG_TX_AGC_B_CCK_11_A_CCK_2_11;
			reg_mask = 0x00007f00;
			break;
		case ODM_RATE5_5M:
			reg_txagc = REG_TX_AGC_B_CCK_11_A_CCK_2_11;
		...
		case ODM_RATE6M:
			reg_txagc = REG_TX_AGC_A_RATE18_06;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATE9M:
			reg_txagc = REG_TX_AGC_A_RATE18_06;
		...
		case ODM_RATE18M:
			...
			break;
		case ODM_RATE24M:
			reg_txagc = REG_TX_AGC_A_RATE54_24;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATE36M:
			reg_txagc = REG_TX_AGC_A_RATE54_24;
		...
		case ODM_RATEMCS0:
			reg_txagc = REG_TX_AGC_A_MCS03_MCS00;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS1:
			reg_txagc = REG_TX_AGC_A_MCS03_MCS00;
		...
		case ODM_RATEMCS3:
			...
			break;
		case ODM_RATEMCS4:
			reg_txagc = REG_TX_AGC_A_MCS07_MCS04;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS5:
			reg_txagc = REG_TX_AGC_A_MCS07_MCS04;
		...
		case ODM_RATEMCS8:
			reg_txagc = REG_TX_AGC_A_MCS11_MCS08;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS9:
			reg_txagc = REG_TX_AGC_A_MCS11_MCS08;
		...
		case ODM_RATEMCS11:
			...
			break;
		case ODM_RATEMCS12:
			reg_txagc = REG_TX_AGC_A_MCS15_MCS12;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS13:
			reg_txagc = REG_TX_AGC_A_MCS15_MCS12;
		...
		switch (hw_rate) {
		case ODM_RATE1M:
			reg_txagc = REG_TX_AGC_B_CCK_1_55_MCS32;
			reg_mask = 0x00007f00;
			break;
		case ODM_RATE2M:
			reg_txagc = REG_TX_AGC_B_CCK_1_55_MCS32;
		...
		case ODM_RATE6M:
			reg_txagc = REG_TX_AGC_B_RATE18_06;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATE9M:
			reg_txagc = REG_TX_AGC_B_RATE18_06;
		...
		case ODM_RATE18M:
			...
			break;
		case ODM_RATE24M:
			reg_txagc = REG_TX_AGC_B_RATE54_24;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATE36M:
			reg_txagc = REG_TX_AGC_B_RATE54_24;
		...
		case ODM_RATEMCS0:
			reg_txagc = REG_TX_AGC_B_MCS03_MCS00;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS1:
			reg_txagc = REG_TX_AGC_B_MCS03_MCS00;
		...
		case ODM_RATEMCS3:
			...
			break;
		case ODM_RATEMCS4:
			reg_txagc = REG_TX_AGC_B_MCS07_MCS04;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS5:
			reg_txagc = REG_TX_AGC_B_MCS07_MCS04;
		...
		case ODM_RATEMCS8:
			reg_txagc = REG_TX_AGC_B_MCS11_MCS08;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS9:
			reg_txagc = REG_TX_AGC_B_MCS11_MCS08;
		...
		case ODM_RATEMCS11:
			...
			break;
		case ODM_RATEMCS12:
			reg_txagc = REG_TX_AGC_B_MCS15_MCS12;
			reg_mask = 0x0000007f;
			break;
		case ODM_RATEMCS13:
			reg_txagc = REG_TX_AGC_B_MCS15_MCS12;
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "config_phydm_read_txagc_n(): path-%d rate index 0x%x = 0x%x\n", path, hw_rate, read_back_data);
