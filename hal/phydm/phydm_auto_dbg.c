...
)
{
	...
	PDM_SNPF(out_len, used, output + used, out_len - used, "PHYDM auto check hang (N-series) is started, Please check the system log\n");
...
)
{
	...
	PDM_SNPF(out_len, used, output + used, out_len - used, "not support now\n");
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "Show dbg port: {1} {1}\n");
		...
		if (var1[1] == 1) {
			if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
				...
				#else
				PDM_SNPF(out_len, used, output + used, out_len - used, "Not support\n");
				#endif
			} else {
				#if (ODM_IC_11N_SERIES_SUPPORT == 1)
