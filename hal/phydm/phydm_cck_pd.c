...
)
{
	...
	PHYDM_DBG(dm, DBG_CCKPD, "%s ======>\n", __func__);
	PHYDM_DBG(dm, DBG_CCKPD, "[92F] pd_th=0x%x, cs_ratio=0x%x, type=0x%x\n\n", cca_pd_th, cca_cs_ratio,type);
	/*
	...
	*/
	switch (type)
		...
		case 0:/*RFBW20_1R*/
		{
			if (cckpd_t->cur_cck_cca_thres != cca_pd_th) {
				cckpd_t->cur_cck_cca_thres = cca_pd_th;
				odm_set_bb_reg(dm, R_0xac8, 0xff, cca_pd_th);
				cckpd_t->cck_fa_ma = CCK_FA_MA_RESET;
			}
			if (cckpd_t->cck_cca_th_aaa != cca_cs_ratio) {
				cckpd_t->cck_cca_th_aaa = cca_cs_ratio;
...
)
{
	...
	if (dm->support_ic_type & EXTEND_CCK_CCATH_AAA_IC) {
		...
		phydm_write_cck_cca_th_92f(dm, (u8)val_buf[0], (u8)val_buf[1],0);
	} else {
...
)
{
	...
	en_2rcca = (u8)(odm_get_bb_reg(dm, R_0xa2c, BIT(18)) && odm_get_bb_reg(dm, R_0xa2c, BIT(22)));
	...
	odm_set_bb_reg(dm, R_0xaa8, 0x1f0000, cs_ration);
	...
	if (en_2rcca == 0) {
		if (dm->is_linked) {
			...
			if ((igi_curr > 0x38) && (dm->rssi_min > 32)) {
				...
				cs_ratio_20m_1r = dig_t->cck_cs_ratio_20m_1r + BASE_92F + STEP_92F * 2;
				cs_ratio_40m_1r = dig_t->cck_cs_ratio_40m_1r + BASE_92F + STEP_92F * 2;
			} else if ((igi_curr > 0x2a) && (dm->rssi_min > 32)) {
				pd_th_20m_1r = dig_t->cck_pd_20m_1r + BASE_92F + STEP_92F;
				pd_th_40m_1r = dig_t->cck_pd_40m_1r + BASE_92F + STEP_92F;
				cs_ratio_20m_1r = dig_t->cck_cs_ratio_20m_1r + BASE_92F + STEP_92F;
				cs_ratio_40m_1r = dig_t->cck_cs_ratio_40m_1r + BASE_92F + STEP_92F;
			} else if ((igi_curr > 0x24) || (dm->rssi_min > 24 && dm->rssi_min <= 30)) {
		...
		PHYDM_DBG(dm, DBG_CCKPD,
		"[92F 1R] pd_th_20m_1r=0x%x, pd_th_40m_1r=0x%x, cs_ratio_20m_1r=0x%x, cs_ratio_40m_1r=0x%x\n", pd_th_20m_1r, pd_th_40m_1r,cs_ratio_20m_1r,cs_ratio_40m_1r);
		phydm_write_cck_cca_th_92f(dm, pd_th_20m_1r, cs_ratio_20m_1r,0);/*1R 20M*/
		phydm_write_cck_cca_th_92f(dm, pd_th_40m_1r, cs_ratio_40m_1r,2);/*1R 40M*/
	}
	else {
		if (dm->is_linked) {
			...
			if ((igi_curr > 0x38) && (dm->rssi_min > 32)) {
				...
				cs_ratio_20m_2r = dig_t->cck_cs_ratio_20m_2r + BASE_92F + STEP_92F * 2;
				cs_ratio_40m_2r = dig_t->cck_cs_ratio_40m_2r + BASE_92F + STEP_92F * 2;
			} else if ((igi_curr > 0x2a) && (dm->rssi_min > 32)) {
				pd_th_20m_2r = dig_t->cck_pd_20m_2r + BASE_92F + STEP_92F;
				pd_th_40m_2r = dig_t->cck_pd_40m_2r + BASE_92F + STEP_92F;
				cs_ratio_20m_2r = dig_t->cck_cs_ratio_20m_2r + BASE_92F + STEP_92F;
				cs_ratio_40m_2r = dig_t->cck_cs_ratio_40m_2r + BASE_92F + STEP_92F;
			} else if ((igi_curr > 0x24) || (dm->rssi_min > 24 && dm->rssi_min <= 30)) {
		...
		PHYDM_DBG(dm, DBG_CCKPD,
		"[92F 2R] pd_th_20m_2r=0x%x, pd_th_40m_2r=0x%x, cs_ratio_20m_2r=0x%x, cs_ratio_40m_2r=0x%x\n", pd_th_20m_2r, pd_th_40m_2r,cs_ratio_20m_2r,cs_ratio_40m_2r);
		phydm_write_cck_cca_th_92f(dm, pd_th_20m_2r, cs_ratio_20m_2r,1);/*2R 20M*/
		phydm_write_cck_cca_th_92f(dm, pd_th_40m_2r, cs_ratio_40m_2r,3);/*2R 40M*/
	}
}
...
)
{
	...
	if (dm->support_ic_type & EXTEND_CCK_CCATH_92F_IC){
		...
		*/
		...
		type=3:  Ad0[29:25]        r_CS_ratio_RFBW40_2R[4:0]
		*/
		dig_t->cck_pd_20m_1r = (u8)odm_get_bb_reg(dm, R_0xac8, 0x000000FF);
		dig_t->cck_pd_20m_2r = (u8)odm_get_bb_reg(dm, R_0xac8, 0x0000ff00);
		dig_t->cck_pd_40m_1r = (u8)odm_get_bb_reg(dm, R_0xacc, 0x000000ff);
		dig_t->cck_pd_40m_2r = (u8)odm_get_bb_reg(dm, R_0xacc, 0x0000ff00);
		dig_t->cck_cs_ratio_20m_1r = (u8)odm_get_bb_reg(dm, R_0xad0,0x0000001f);
		dig_t->cck_cs_ratio_20m_2r = (u8)odm_get_bb_reg(dm, R_0xad0,0x000003e0);
		dig_t->cck_cs_ratio_40m_1r = (u8)odm_get_bb_reg(dm, R_0xad0,0x01f00000);
		dig_t->cck_cs_ratio_40m_2r = (u8)odm_get_bb_reg(dm, R_0xad0,0x3e000000);
	}
