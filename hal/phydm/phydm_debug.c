...
phydm_bb_dbg_port_header_sel(
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(dm, R_0x8f8, (BIT(25) | BIT(24) | BIT(23) | BIT(22)), header_idx);
...
phydm_bb_dbg_port_clock_en(
	...
	if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C | ODM_RTL8814A | ODM_RTL8814B)) {
		...
		odm_set_bb_reg(dm, R_0x198c, 0x7, reg_value); /*enable/disable debug port clock, for power saving*/
	}
}
...
)
{
	...
	PDM_SNPF(out_len, used, output + used, out_len - used, "\r\n %-35s = %d / %d", "value SCFO(Hz) <A/B>",
		scfo_a, scfo_b);
	PDM_SNPF(out_len, used, output + used, out_len - used, "\r\n %-35s = %d / %d", "Avg CFO(Hz) <A/B>",
		avg_cfo_a, avg_cfo_b);
	...
	PDM_SNPF(out_len, used, output + used, out_len - used, "\r\n %-35s = %d / %d", "End CFO(Hz) <A/B>",
		cfo_end_a, cfo_end_b);
	PDM_SNPF(out_len, used, output + used, out_len - used, "\r\n %-35s = %d / %d", "ACQ CFO(Hz) <A/B>",
		acq_cfo_a, acq_cfo_b);
}
...
)
{
	...
	if (rx_ht != 0) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used, "  %-35s", tmp_string);
	}
	...
	if (dm->support_ic_type & ODM_RTL8822B) {
		...
		odm_set_bb_reg(dm, R_0x1988, BIT(22), 0x1);	/*enable report condition number*/
...
/*jj add 20170822*/
	...
	PDM_SNPF(out_len, used, output + used, out_len - used,"  %-35s: %d (Subversion: %d)\n", "FW Ver",
...
)
{
	...
	if (dm_value[1] >= dm->num_rf_path) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "Write path-%d rate_idx-0x%x fail\n",
		...
		if (phydm_api_set_txagc(dm, dm_value[3], (enum rf_path) dm_value[1], (u8)dm_value[2], true))
			PDM_SNPF(out_len, used, output + used, out_len - used, "Write path-%d rate_idx-0x%x = 0x%x\n",
				 dm_value[1], dm_value[2], dm_value[3]);
		else
			PDM_SNPF(out_len, used, output + used, out_len - used, "Write path-%d rate index-0x%x fail\n",
				 dm_value[1], dm_value[2]);
	} else {
		...
		if (status)
			PDM_SNPF(out_len, used, output + used, out_len - used, "Write all TXAGC of path-%d = 0x%x\n",
				 dm_value[1], dm_value[3]);
		else
			PDM_SNPF(out_len, used, output + used, out_len - used, "Write all TXAGC of path-%d fail\n",
				 dm_value[1]);
	}
...
)
{
	...
	for (addr = 0x800; addr < 0xfff; addr += 4) {
		PDM_VAST_SNPF(output, output + used, out_len - used, "0x%03x 0x%08x\n",
...
)
{
	...
	for (addr = 0x800; addr < 0xfff; addr += 4) {
		PDM_VAST_SNPF(output, output + used, out_len - used, "0x%04x 0x%08x\n",
			addr, odm_get_bb_reg(dm, addr, MASKDWORD));
	}
...
)
{
	...
	PDM_VAST_SNPF(output, output + used, out_len - used, "BB==========\n");
...
)
{
	...
	/* dump RF register */
	PDM_VAST_SNPF(output, output + used, out_len - used, "RF-A==========\n");
	for (addr = 0; addr < 0xFF; addr++)
		PDM_VAST_SNPF(output, output + used, out_len - used, "0x%02x 0x%05x\n",
			addr, odm_get_rf_reg(dm, RF_PATH_A, addr, RFREGOFFSETMASK));
#ifdef PHYDM_COMPILE_ABOVE_2SS
	if (dm->rf_type > RF_1T1R) {
		PDM_VAST_SNPF(output, output + used, out_len - used, "RF-B==========\n");
		for (addr = 0; addr < 0xFF; addr++)
			PDM_VAST_SNPF(output, output + used, out_len - used, "0x%02x 0x%05x\n",
	...
	if (dm->rf_type > RF_2T2R) {
		PDM_VAST_SNPF(output, output + used, out_len - used, "RF-C==========\n");
		for (addr = 0; addr < 0xFF; addr++)
			PDM_VAST_SNPF(output, output + used, out_len - used, "0x%02x 0x%05x\n",
	...
	if (dm->rf_type > RF_3T3R) {
		PDM_VAST_SNPF(output, output + used, out_len - used, "RF-D==========\n");
		for (addr = 0; addr < 0xFF; addr++)
			PDM_VAST_SNPF(output, output + used, out_len - used, "0x%02x 0x%05x\n",
...
)
{
	...
	/* dump MAC register */
	PDM_VAST_SNPF(output, output + used, out_len - used, "MAC==========\n");
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "dumpreg {0:all, 1:BB, 2:RF, 3:MAC}\n");
	} else if (var1[0] == 0) {
