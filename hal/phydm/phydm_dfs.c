	...
	if (dm->support_ic_type & (ODM_RTL8821 | ODM_RTL8812 | ODM_RTL8881A)) {
		...
		if (region_domain == PHYDM_DFS_DOMAIN_ETSI) {
			...
			if (c_channel >= 52 && c_channel <= 64) {
				odm_set_bb_reg(dm, R_0x918, MASKDWORD, 0x1c16ecdf);
				odm_set_bb_reg(dm, R_0x91c, MASKDWORD, 0x0f141a20);
			} else {
				odm_set_bb_reg(dm, R_0x918, MASKDWORD, 0x1c16acdf);
				if (band_width == CHANNEL_WIDTH_20)
					odm_set_bb_reg(dm, R_0x91c, MASKDWORD, 0x64721a20);
				else
					odm_set_bb_reg(dm, R_0x91c, MASKDWORD, 0x68721a20);
			}
			...
			if (band_width == CHANNEL_WIDTH_20)
				odm_set_bb_reg(dm, R_0x91c, MASKDWORD, 0x64741a20);
			else
				odm_set_bb_reg(dm, R_0x91c, MASKDWORD, 0x68741a20);
