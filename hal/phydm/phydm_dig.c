...
)
{
	...
	#if (RTL8192F_SUPPORT == 1 && RTL8192F_A_CUT==1)/*jj add 20170822*/
	if (dm->support_ic_type & ODM_RTL8192F)/*jj add 20170822*/
		if (current_igi > 0x38) {
			current_igi = 0x38;
			PHYDM_DBG(dm, DBG_DIG,
				"Force to RTL8192F Adaptivity Upper bound=((0x%x))\n", current_igi);
		}
	#endif
	...
	if (dig_t->cur_ig_value != current_igi) {
		...
		if (dm->cck_new_agc && (dm->support_ic_type & PHYSTS_2ND_TYPE_IC))
			odm_set_bb_reg(dm, R_0xa0c, 0x3f00, (current_igi >> 1));
		#endif
...
#ifdef PHYDM_DIG_MODE_DECISION_SUPPORT	/*AP only*/
	struct phydm_dig_struct	*dig_t = &dm->dm_dig_table;
...
)
{
	...
	} else if (!is_performance) {	/*AP only*/
		dig_t->dm_dig_max = DIG_MAX_COVERAGR;
		dig_t->dm_dig_min = DIG_MIN_COVERAGE;
		#if (DIG_HW == 1)
...
)
{
	...
	PHYDM_DBG(dm, DBG_DIG, "is_linked = %d, RSSI = %d, 1stConnect = %d, 1stDisconnect = %d\n",
...
)
{
	...
	if (dm->support_ic_type != ODM_RTL8723D) {
		...
		false_alm_cnt->edcca_flag = (boolean)(odm_get_bb_reg(dm, R_0x9a0, BIT(29)));
	}
...
)
{
	...
	return (u1Byte)odm_get_bb_reg(dm, R_0xc70, 0x1e00);
}
	...
	)
{
	...
	u32			sat_status_patha, sat_status_pathb;
	u8			igi_restore = dig_t->cur_ig_value;
	u8			i, lna_sat_chk_cnt = dm->lna_sat_chk_cnt;
	...
	for (i = 0; i < lna_sat_chk_cnt; i++) {
		sat_status_patha = odm_get_rf_reg(dm, RF_PATH_A, RF_0xae, 0xc0000);
		sat_status_pathb = odm_get_rf_reg(dm, RF_PATH_B, RF_0xae, 0xc0000);
