	...
	for (path = 0; path <= max_path; path++)
		for (rate_idx = 0; rate_idx < 84; rate_idx++)
			if (phydm_check_rates(dm, rate_idx)) {
				...
				PHYDM_DBG(dm, DBG_DYN_TXPWR, "Support Rate: ((%d)) -> Gain idx: ((%d))\n", rate_idx, gain_index);
...
)
{
	...
	PHYDM_DBG(dm, DBG_DYN_TXPWR, "DTP hp_th: Lv1_th =%d ,Lv2_th = %d ,Lv3_th = %d\n", dm->enhance_pwr_th[0], dm->enhance_pwr_th[1], dm->enhance_pwr_th[2]);
}
...
)
{
	...
	PHYDM_DBG(dm, DBG_DYN_TXPWR, "RespPwr update_DTP_lv: ((%d)) -> ((%d))\n", dm->last_dtp_lvl, dm->dynamic_tx_high_power_lvl);
	dm->last_dtp_lvl = dm->dynamic_tx_high_power_lvl;
...
)
{
	...
	PHYDM_DBG(dm, DBG_DYN_TXPWR, "Fill cmninfo TxPwr: macid=(%d), PwrLv (%d)\n", macid, dtp->dyn_tx_power);
...
)
{
	...
	if (is_sta_active(sta)) {
		...
		PHYDM_DBG(dm, DBG_DYN_TXPWR, "STA=%d , RSSI: %d , GetPwrLv: %d\n", macid, rssi->rssi, dtp->sta_tx_high_power_lvl);
		if (dtp->sta_tx_high_power_lvl == tx_high_pwr_level_unchange || dtp->sta_tx_high_power_lvl == dtp->sta_last_dtp_lvl) {
			...
			PHYDM_DBG(dm, DBG_DYN_TXPWR, "DTP_lv not change: ((%d))\n",dtp->sta_tx_high_power_lvl);
			return;
		}

		PHYDM_DBG(dm, DBG_DYN_TXPWR, "DTP_lv update: ((%d)) -> ((%d))\n", dm->last_dtp_lvl, dm->dynamic_tx_high_power_lvl);
...
)
{
	...
	PHYDM_DBG(dm, DBG_DYN_TXPWR, "%s: channel = %d  TXpower lvl=%d/%d\n",
		__func__, hal_data->CurrentChannel, hal_data->LastDTPLvl, hal_data->DynamicTxHighPowerLvl);
...
)
{
	...
	if (dm ->fill_desc_dyntxpwr)
		...
		PHYDM_DBG(dm, DBG_DYN_TXPWR, "odm_set_dyntxpwr: fill_desc_dyntxpwr is null!\n");
	if (dtp->last_tx_power != dtp->dyn_tx_power) {
		PHYDM_DBG(dm, DBG_DYN_TXPWR, "odm_set_dyntxpwr: last_offset=%d, txpwr_offset=%d\n",
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PDM_SNPF(out_len, used, output + used, out_len - used, "{DynTxPwr} {TH1 TH2 TH3} \n");
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			       "Phydm Set DTP : TH1 = (( 0x%x)), TH2 = (( 0x%x)), TH3 = (( 0x%x))\n", dm->set_pwr_th[0], dm->set_pwr_th[1], dm->set_pwr_th[2]);
	}
#else
	PDM_SNPF(out_len, used, output + used, out_len - used,
...
)
{
	...
	if (!(dm->support_ability & ODM_BB_DYNAMIC_TXPWR))
		return;
	PHYDM_DBG(dm, DBG_DYN_TXPWR, "Dynamic TxPwr : RSSI_min = %d, Noisy_dec = %d\n", rssi_min, dm->noisy_decision);
