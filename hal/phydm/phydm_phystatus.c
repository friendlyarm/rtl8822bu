...
void
phydm_get_rx_phy_status_3RD_type0(
	struct dm_struct		*dm,
	u8							*phy_status_inf,
	struct phydm_perpkt_info_struct	*pktinfo,
	struct phydm_phyinfo_struct	*phy_info
)
{
	...
	phydm_set_common_phy_info(rx_power[0], phy_sta_rpt->channel, false,
		  false, CHANNEL_WIDTH_20, sq, phy_sta_rpt->l_rxsc, phy_info);

	/* Update CCK pwdb */
	phydm_set_per_path_phy_info(RF_PATH_A, rx_power[0], 0, 0, 0, phy_info);					/* Update per-path information */
	phydm_set_per_path_phy_info(RF_PATH_B, rx_power[1], 0, 0, 0, phy_info);
	phydm_set_per_path_phy_info(RF_PATH_C, rx_power[2], 0, 0, 0, phy_info);
	phydm_set_per_path_phy_info(RF_PATH_D, rx_power[3], 0, 0, 0, phy_info);
...
void
phydm_get_rx_phy_status_3RD_type1(
	struct dm_struct		*dm,
	u8							*phy_status_inf,
	struct phydm_perpkt_info_struct	*pktinfo,
	struct phydm_phyinfo_struct	*phy_info
)
{
	...
	for (i = RF_PATH_A; i < PHYDM_MAX_RF_PATH; i++) {
		if (dm->rx_ant_status & BIT(i)) {
			...
			/*phydm_set_per_path_phy_info(i, rx_path_pwr_db, phy_sta_rpt->rxevm[i],
	...
	phydm_set_common_phy_info(rx_pwr_db, phy_sta_rpt->channel_pri_lsb, (boolean)phy_sta_rpt->beamformed,
...
void
phydm_process_rssi_for_dm_3rd_type(
	struct dm_struct		*dm,
	struct phydm_phyinfo_struct	*phy_info,
	struct phydm_perpkt_info_struct	*pktinfo
)
...
)
{
...
#if 0 /*def PHYDM_PHYSTAUS_SMP_MODE*/
	struct pkt_process_info			*pkt_process = &dm->pkt_proc_struct;
#endif
	u8		phy_status_type = (*phy_status_inf & 0xf);

	#if 0 /*[Step 3]*//*def PHYDM_PHYSTAUS_SMP_MODE*/
	...
	case 1:
		...
		break;
	case 2:
		phydm_get_physts_jarguar3_2(dm, phy_status_inf, pktinfo, phy_info);
		break;
	default:
		break;
	}
