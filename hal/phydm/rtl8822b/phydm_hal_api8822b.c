...
)
{
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "RFE[%d]:{Path=0x%x}{inv_en=%d}{source=0x%x}\n",
	...
	if(rfe_num > 11) {
	 ...
	 /*[Path_mux_sel]*/
	odm_set_bb_reg(dm, R_0x1990, BIT(rfe_num), ((path_mux_sel == BB_PATH_A) ? 0 : 1));

	/*[Inv_en]*/
	odm_set_bb_reg(dm, R_0xcbc, BIT(rfe_num), (u32)inv_en);
	odm_set_bb_reg(dm, R_0xebc, BIT(rfe_num), (u32)inv_en);

	/*[Output Source Signal Selection]*/
	if (rfe_num <= 7) {
		odm_set_bb_reg(dm, R_0xcb0, ((0xf)<<(rfe_num * 4)), (u32)source_sel);
		odm_set_bb_reg(dm, R_0xeb0, ((0xf)<<(rfe_num * 4)), (u32)source_sel);
	} else {
		odm_set_bb_reg(dm, R_0xcb4, ((0xf)<<((rfe_num - 8) * 4)), (u32)source_sel);
		odm_set_bb_reg(dm, R_0xeb4, ((0xf)<<((rfe_num - 8) * 4)), (u32)source_sel);
	}
}
...
)
{
	...
	if ((rfe_type == 4) || (rfe_type == 11)) {
		...
		if ((dm->cut_version == ODM_CUT_B) && (rfe_type < 2)) {
			...
			/* delay 400ns for PAPE */
			/* odm_set_bb_reg(dm, R_0x810, MASKBYTE3|BIT20|BIT21|BIT22|BIT23, 0x211); */
...
)
{
	...
	if (dm->cut_version == ODM_CUT_B) {
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "Update CCA para for Bcut\n");
	} else
#endif
	{
...
)
{
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		"RFE type (%d), Board type (0x%x), Package type (%d)\n", dm->rfe_type, dm->board_type, dm->package_type);
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		"5G ePA (%d), 5G eLNA (%d), 2G ePA (%d), 2G eLNA (%d)\n", dm->ext_pa_5g, dm->ext_lna_5g, dm->ext_pa, dm->ext_lna);
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		"5G PA type (%d), 5G LNA type (%d), 2G PA type (%d), 2G LNA type (%d)\n", dm->type_apa, dm->type_alna, dm->type_gpa, dm->type_glna);
...
)
{
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "RF-%d 0x%x = 0x%x (original: 0x%x), bit mask = 0x%x\n",
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_API, "max_ret_psd_final = %d, max_ret_psd_b_final = %d\n", max_ret_psd_final, max_ret_psd_b_final);
...
)
{
	...
	if (rf_reg_status == false) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "Fail to switch band (ch: %d), write RF_reg fail\n", central_ch);
		return false;
	}
...
)
{
	...
	if (enable_rxagc_switch == true) {
		if ((*dm->channel >= 36) && (*dm->channel <= 64)) {
			...
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x1);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x5);
		} else if ((*dm->channel >= 100) && (*dm->channel <= 144)) {
			odm_set_bb_reg(dm, R_0x958, BIT(4), 0x1);
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x2);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x6);
		} else if (*dm->channel >= 149) {
			odm_set_bb_reg(dm, R_0x958, BIT(4), 0x1);
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x3);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x7);
		}
		...
		if ((*dm->channel >= 36) && (*dm->channel <= 64)) {
			odm_set_bb_reg(dm, R_0x958, BIT(4), 0x1);
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x1);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x1);
		} else if ((*dm->channel >= 100) && (*dm->channel <= 144)) {
			odm_set_bb_reg(dm, R_0x958, BIT(4), 0x1);
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x2);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x2);
		} else if (*dm->channel >= 149) {
			odm_set_bb_reg(dm, R_0x958, BIT(4), 0x1);
			odm_set_bb_reg(dm, R_0xc1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x3);
			odm_set_bb_reg(dm, R_0xe1c, (BIT(11)|BIT(10)|BIT(9)|BIT(8)), 0x3);
		}
...
)
{
	...
	if (rf_reg_be != 0xff)
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xbe, (BIT(17) | BIT(16) | BIT(15)), rf_reg_be);
	else {
...
)
{
	...
	odm_set_bb_reg(dm, R_0x808, MASKBYTE0, (dm->rx_ant_status | (dm->rx_ant_status << 4)));
...
)
{
	...
	odm_set_bb_reg(dm, R_0x80c, MASKBYTE0, ((tx_path << 4) | tx_path));/* TX path HW block enable */

	/* Tx2path for 1ss */
	if (!((tx_path == BB_PATH_A) || (tx_path == BB_PATH_B))) {
		if (is_tx2_path || *dm->mp_mode) {

			odm_set_bb_reg(dm, R_0x93c, 0xfff00000, 0x043);	/* 2Tx for OFDM */
			odm_set_bb_reg(dm, R_0xa04, 0xf0000000, 0xc);	/* 2Tx for CCK */
		}
	}

	/*[RX Antenna Setting] ==========================================*/

	odm_set_bb_reg(dm, R_0xa2c, BIT(22), 0x0);	/*Disable MRC for CCK CCA */
	odm_set_bb_reg(dm, R_0xa2c, BIT(18), 0x0);	/*Disable MRC for CCK barker */
	...
	if ((rx_path == BB_PATH_A) || (rx_path == BB_PATH_B)) {
		...
		odm_set_bb_reg(dm, R_0x1904, BIT(16), 0x0);	/*AntWgt_en*/
		odm_set_bb_reg(dm, R_0x800, BIT(28), 0x0);	/*htstf ant-wgt enable = 0*/
		odm_set_bb_reg(dm, R_0x850, BIT(23), 0x0);	/*MRC_mode  =  'original ZF eqz'*/
	} else {
		/* 2R */
		...
		odm_set_bb_reg(dm, R_0x1904, BIT(16), 0x1);	/*AntWgt_en*/
		odm_set_bb_reg(dm, R_0x800, BIT(28), 0x1);	/*htstf ant-wgt enable = 1*/
		odm_set_bb_reg(dm, R_0x850, BIT(23), 0x1);	/*MRC_mode =  'modified ZF eqz'*/
	}
	...
	while (1) {
		...
		if ((rf_reg33 == 0x00001) && (config_phydm_read_rf_check_8822b(rf_reg33)))
			...
			PHYDM_DBG(dm, ODM_PHY_CONFIG, "Fail, write RF mode table fail\n");
			return false;
		}
	}
	...
	if (*dm->mp_mode || (*dm->antenna_test) || (dm->normal_rx_path)) {
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "MP/Ant test mode, support path-B TRX\n");
	} else {
		...
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, RFREGOFFSETMASK, 0x00000);
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "Normal mode, not support path-B TRX\n");
	}
...
)
{
	if (type == ODM_PRE_SETTING) {
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "Pre set: disable OFDM/CCK block\n");
	} else if (type == ODM_POST_SETTING) {
		odm_set_bb_reg(dm, R_0x808, (BIT(28) | BIT(29)), 0x3);
		PHYDM_DBG(dm, ODM_PHY_CONFIG, "Post set: enable OFDM/CCK block\n");
#if (PHYDM_FW_API_FUNC_ENABLE_8822B == 1)
